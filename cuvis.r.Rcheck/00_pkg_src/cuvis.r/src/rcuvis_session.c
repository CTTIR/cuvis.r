...
#ifndef CUVIS_STUB
    int handle = (int)(intptr_t)rcuvis_get_handle(session_xp, cuvis_session_tag,
                                                   "session");
    int itype = INTEGER(item_type)[0];
    int size = 0;
...
SEXP rcuvis_session_get_mesu(SEXP session_xp, SEXP index, SEXP item_type) {
    ...
    int idx = INTEGER(index)[0];  /* Already 0-based from R side */
    int itype = INTEGER(item_type)[0];
    int mesu_handle = 0;
    ...
    /* Use measurement finalizer declared in rcuvis_measurement.c */
...
SEXP rcuvis_session_get_hash(SEXP session_xp) {
    ...
    int handle = (int)(intptr_t)rcuvis_get_handle(session_xp, cuvis_session_tag,
                                                   "session");
    const char *hash = cuvis_session_file_get_hash_swig(handle);
    return Rf_ScalarString(Rf_mkChar(hash ? hash : ""));
#else
    Rf_error("cuvis.r was installed without the CUVIS SDK.");
    return R_NilValue;
#endif
